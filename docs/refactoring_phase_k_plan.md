...
> **Status:** IN PROGRESS — K0 through K13 complete; K14a (`454fd0e`), K14b (`c6b4b67`), K14c (`1559fc4`), K14d (`54df444`), K14e (`06438e7`), K14f (`d99ca32`), K14g (`33e697f`) committed.
> **Drafted:** 2026-05-08
...
suite (144 scripts / 2 908 tests / 5 503 asserts / 0 failures)**. It introduces
...
| **K14** | Extract `AttackFlowExecutor` (RefCounted) into `src/core/combat/`. K14a committed `454fd0e`: moved attack-flow payload builders (`compute_attack_identity_patch`, clear-target patch) into core helper, routed `attack_executor.gd` to delegate these builders, and added `test_attack_flow_executor.gd` with singleton-state isolation hooks. K14b committed `c6b4b67`: moved attack-state init/reset/roll parsing/defense payload builders into core helper and delegated corresponding call sites. K14c committed `1559fc4`: moved defense-commit canonical ordering + queue initialization helpers into core helper. K14d committed `54df444`: moved defense-queue polling + faceup-card counting into core helper and delegated corresponding scene-layer paths. K14e committed `06438e7`: moved first-faceup decision + damage-summary construction into core helper and delegated corresponding scene-layer helpers. K14f committed `d99ca32`: moved redirect-continuation decision into core helper and delegated corresponding `attack_executor.gd` helper. K14g committed `33e697f`: moved faceup-card preparation + immediate-effect flow decision into core helper, delegated corresponding scene-layer call sites, and expanded `test_attack_flow_executor.gd`. | high | +900 / −800 | yes |
| **K15** | Function-size + nesting cleanup pass on attack_executor.gd remainder. Extracted 16 private helpers (`_build_immediate_choice_payload`, `_dispatch_immediate_choice_network`, `_try_open_immediate_choice_handoff`, `_publish_attack_declare_patch`, `_handle_empty_attack_pool`, `_try_offer_cf_dial`, `_resolve_defense_step_defender`, `_show_defense_section`, `_finish_redirect_substep`, `_emit_redirect_shield_refresh`, `_canonicalise_committed_tokens`, `_resolve_zero_damage`, `_continue_ship_damage_resolution`, `_show_next_squadron_panel_prompt`, `_publish_redirect_substep_patch`, `_draw_structural_damage_extra`, `_auto_resolve_immediate_effect`, `_refresh_post_evade_panel`, `_wire_attack_done_and_panel_signals`, `_auto_skip_ship_attack`) to bring every function under the 30-line cap (excluding doc-comments and blanks per `.skills/refactoring_guidelines.md`). Preserves the brace canonical-sort fix from `c673ef0` (now isolated inside `_canonicalise_committed_tokens`) and the structural-damage extra-card draw (now isolated inside `_draw_structural_damage_extra`). attack_executor.gd: 2 384 → 2 484 LOC (LOC grows because each extraction adds a signature + docstring; per-function code stays under 30). Committed `<pending>`. | medium | +100 / 0 | no |
