...
| `src/autoload/game_manager.gd` | Preserve a saved canonical Ship-phase `WAIT_FOR_SHIP_SELECT` controller when restoring active-player presentation, after the existing active-ship recovery takes precedence; hydrate only an already-public dial face supplied by an accepted authoritative command result. This adds no publication, assignment, or canonical-owner responsibility. |
| `src/core/commands/activate_ship_command.gd` | Reject a ship-selection command from a player other than the canonical `WAIT_FOR_SHIP_SELECT` controller. |
| `src/core/commands/reveal_dial_command.gd` | Reject a ship-selection command from a player other than the canonical `WAIT_FOR_SHIP_SELECT` controller. |
| `src/core/commands/convert_dial_to_token_command.gd` | Reject a ship-selection command from a player other than the canonical `WAIT_FOR_SHIP_SELECT` controller. |
| `src/core/state/command_dial_stack.gd` | Represent an intentionally redacted opponent dial safely and hydrate its revealed face only from an accepted authoritative result. |
| `src/scenes/game_board/ui_panel_manager.gd` | On an explicit return to the main menu, close the live Network session so it cannot be treated as an in-game confirmed-loss reconnect; do not alter ordinary in-game disconnect handling. |

No production change is authorized in `GameState`,
`MatchPlayerControlBinding`, `StateFilter`, `UIProjector`, save/replay models,
integrity helpers, player profiles, or generic session/controller abstractions.
The six narrowly listed recovery, redaction, and explicit-quit lifecycle paths
above are the complete exception; they do not authorize further gameplay or
...
| `tests/acceptance/network_resume/network_replay_v7.json` | Persisted format-7 two-human Network replay fixture consumed by the process-isolated compatibility scenario; prove protocol-4 replay transport has no fresh-resume assignment traffic. |
...
9. structural searches proving `GameState` and binding have no new rebind/kind/
   principal mutation, no production diff exists in `StateFilter` or save/replay
   schemas, and any diff in the six Section 11.1 recovery/redaction/quit paths
   remains confined to its listed purpose;
...
- production changes to `StateFilter`, `UIProjector`, `GameState`, binding,
  save/replay schema, command schema, or accepted gameplay owners beyond the
  six exact Section 11.1 recovery/redaction/explicit-quit exceptions;
