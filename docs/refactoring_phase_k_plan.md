...
| **K13** | Function-size cleanup pass on what remains in game_board.gd. Landed as helper extraction + dispatch simplification in-place (no new controller/class extraction); all game_board functions now <= 30 LOC. Committed `<pending>`. | medium | +276 / -289 | no |
