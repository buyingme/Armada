...
| Status | Implemented foundation as of 2026-06-07: setup-package bootstrap now writes serialized setup interaction-flow rows for obstacle placement, ship deployment, squadron deployment, and setup review; setup commands recompute flow authority after each committed mutation; runtime setup payloads now preserve player display names for projection; and `SetupPlacementController` reads interactivity from `UIProjector` instead of plain `SETUP`-phase checks. Remaining presentation work for handoff banners/lower-middle modal layout belongs to FB14F/FB14H, and the command-backed review readiness gate remains FB14I. |
...
| Scope | Refactor the existing board setup panel into a projection-driven lower-middle obstacle modal. Show the available obstacle list during the obstacle step, disable already placed obstacles, and use a two-stage preview flow: selecting an obstacle arms placement, the first board click drops a movable preview token, clicking the preview again re-enters move mode, and only an explicit modal `confirm placement` action commits through `commit_setup_obstacle`. Preview rotation uses the same input method as debug token rotation. Hot-seat rotates/announces `Player Name place obstacle` between placements. Network keeps each peer on their own perspective, uses the normal network turn-transition/passive-view pattern, and shows only committed obstacle placements to passive peers. Add setup-area and deployment-zone overlays for setup presentation, and keep obstacle placement outside deployment zones on both 3x3 and 3x6 maps. |
...
| Acceptance | Players can place and rotate all six obstacles in order; board clicks only drop or resume moving the preview; only the modal confirm button commits placement and advances the flow; previews stay outside deployment zones; rejected previews/commits show core validation errors with overlap-style legality highlighting; setup-area and deployment-zone overlays are visible during setup; inactive network peers see each committed obstacle without gaining control or live drag authority. |
| Tests | UI construction, lower-middle obstacle modal, obstacle button availability, preview drop/reselect interaction, explicit confirm button commit, rotation input, command payload normalization, legality-highlight projection, deployment-zone blocking on 3x3 and 3x6, passive-peer committed-only visibility, setup/deployment overlay visibility, projection-driven enabled/disabled state, no PlayMode lint violations. |
...
| Status | Implemented as of 2026-06-07: extracted a bottom-centre `SetupPlacementModal`, moved obstacle selection into a projection-driven live preview flow with magnify-gesture rotation, surfaced validator-backed legality feedback on the preview token/modal, and added hot-seat setup handoff prompts while keeping passive network peers committed-only. Focused controller GUT, full GUT, Phase K lint, and replay/network baseline traces pass. |
