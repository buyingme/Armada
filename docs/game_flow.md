...
"blinded_gunners"`, but `GameState.effect_registry` was empty after
deserialize, so the `ATTACK_SPEND_ACCURACY` blocker did not fire. The invariant
for M is now explicit: after `deserialize -> start_new_game_from_state ->
EffectFactory.rebuild_runtime_effects`, the same faceup card must block
accuracy spending in hot-seat, network, and replay. If Blinded Gunners later
...
| M11 | Crew Panic | `BEFORE_REVEAL_DIAL` plus scene-side choice modal resolves dial discard or facedown damage. | `RuleRegistry` ENABLER that exposes the choice affordance through `UIIntent.affordances`; final mutation remains command-backed. | Bridge required until M11 replaces the scene-side modal/effect-resolution path. Target after M11 is pure active-card lookup from `faceup_damage`. |
| M12 | Capacitor Failure | `DEFENSE_VALIDATE_TOKEN` blocks redirect and `REPAIR_VALIDATE_SHIELD` blocks shield repair into zones with 0 shields. | Pure multi-hook rule: defense-token validator plus repair/shield blocker from current shields. | No bridge after migration. Until M12, legacy `EffectRegistry` remains the compatibility path. |
...
| `status_phase_cleanup` | `PHASE` | `STATUS` | Deterministic end-of-round cleanup, including legacy `STATUS_READY_TOKENS` effects. |
...
| `spend_dial` | `PHASE` | `SHIP` | Used by activation, Concentrate Fire, and Crew Panic fallback paths. M11 can tighten this after Crew Panic becomes a projected ENABLER. |
...
| `roll_dice` | `FLOW_STEP` | `ATTACK / ATTACK_ROLL` | Attack dice roll is meaningful only inside the attack roll step. |
...
| `persistent_effect_damage` | `PHASE` | `SHIP` | Legacy persistent effects such as Crew Panic and overlap/maneuver damage cards are not all represented by a single flow step yet. |
