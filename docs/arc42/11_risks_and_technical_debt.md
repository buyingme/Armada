...
| R-6 | God-object files resist extension | High | High | Refactoring plan Phases A–F (A–E ✅, F partially ✅). See `docs/refactoring_plan.md`. |
...
| TD-8 | `attack_executor.gd` is 3 285 lines / 146 functions — single state machine. F4 deferred: 159 `_attack_sim_panel` refs across 80 functions make façade extraction impractical. | **High** | Phase A shrunk functions. F4 deferred. Future: incrementally extract subsystems (dice UI, defense token UI) when attack flow is next modified. |
...
> **Last audit:** Refactoring Phase F (partial) complete — 92 scripts, 1 754 tests, 3 113 asserts, all passing.
