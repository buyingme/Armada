...
## Builds a serialized persistent damage card (Blinded Gunners).
func _make_persistent_card_data() -> Dictionary:
	...
	card.effect_text = "Cannot spend accuracy icons."
	card.is_faceup = true
...
func test_execute_adds_faceup_card() -> void:
	...
	assert_eq(card.effect_id, "blinded_gunners",
			"Card effect_id should match")
	assert_eq(card.title, "Blinded Gunners",
			"Card title should match")
	assert_eq(result.get("effect_id"), "blinded_gunners",
			"Result should contain effect_id")
	assert_eq(result.get("card_title"), "Blinded Gunners",
...
func test_roundtrip() -> void:
	...
	assert_eq(restored.payload.get("effect_id"), "blinded_gunners",
	...
	assert_eq(restored_card.get("effect_id"), "blinded_gunners",
