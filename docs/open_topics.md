...
| I6e-2 | Relocate boot config source (L193) + host-only fixed round-1 (L459) out of `game_board.gd` into `GameManager.start_new_game` / `apply_fixed_round1_commands` | ✅ `37bf448` — `GameManager.bootstrap_game(default_scenario_id)` handles network/hot-seat config split; network-client gate moved into `apply_fixed_round1_commands`. 9 → 7 real branches. |
| I6e-3 | Name the submitter-contract pattern (`result.is_empty()` + `is_network()`) at L1261 / L2651 | ✅ `e158c34` — `_is_pending_remote_result(result)` + `_is_local_command_rejection(result)` helpers; both call sites replaced. 7 → 5 real branches. |
| I6b-4a | Squadron-displacement domain plumbing: `Constants.InteractionFlow.SQUADRON_DISPLACEMENT` + `InteractionStep.DISPLACEMENT_PLACE`; `StartDisplacementCommand` + `CommitDisplacementCommand` (RefCounted, full validate/execute/serialize); registered + tested. **Not yet wired** — runtime still calls `_displacement_controller.start()` directly. | ✅ commit pending — 16 new tests, 30 new asserts (133 / 2 753 / 5 150). MT-PHI.06b-4a pending (regression-only: hot-seat + network displacement still works via the legacy path; new commands are dead code until I6b-4c). |
| I6b-4b | Squadron-displacement projection: `UIProjector` returns `ModalKind.DISPLACEMENT` when flow is active and viewer is controller; `DisplacementController` subscribes to projection. | ✅ commit pending — `Constants.ModalKind.DISPLACEMENT` added; `UIProjector._modal_kind_for` extended (1 line); 3 new projector tests. Controller subscription deferred to I6b-4c with the call-site swap so the dual-driver risk is removed. 133 / 2 756 / 5 158. **No MT needed** — pure projection, no runtime call path changed. |
| I6b-4c | Squadron-displacement call-site swap: `_finalize_maneuver_execute` submits `StartDisplacementCommand`; `DisplacementController._on_committed` submits `CommitDisplacementCommand`. | I6b-4c-1 ✅ committed `647af38` — both commands submitted alongside legacy direct call. I6b-4c-2 ✅ committed `f898805` — OV-002 fix: in network mode the legacy direct `_displacement_controller.start()` is gated to hot-seat only; the modal opens on the squadron-owner peer via projection (`_open_displacement_modal_from_command` reacts to `start_displacement` broadcast). Maneuvering peer's End-Activation resume on `commit_displacement` broadcast via `_resume_after_remote_displacement`. Hot-seat behaviour unchanged. 133 / 2 754 / 5 158. MT-PHI.06b-4c-1 ✅ + MT-PHI.06b-4c-2 ✅ — both verified by user ("network and hot seat work perfectly"). |
| I6b-4d | Squadron-displacement cleanup: drop redundant per-squadron `submit_move_squadron` calls during displacement; visual sync on the maneuvering peer now flows through a single `commit_displacement` broadcast. Adds `_handle_remote_commit_displacement` (emits `squadron_repositioned_remotely` per placement) and silences the `start_displacement` / `commit_displacement` *Unhandled remote command type* warnings. | ✅ commit pending — 133 / 2 756 / 5 158 (unchanged). MT-PHI.06b-4d pending (regression-only: hot-seat + network displacement). |
| I6e-3 | Collapse asymmetric activation result-handling (L1257/L1270) + debug-damage rejection log (L2620) by introducing an `awaiting_remote: bool` sentinel on result dicts | ✅ commit `52918b3` — `NetworkCommandSubmitter.submit()` now returns `{"awaiting_remote": true}` (constant `AWAITING_REMOTE_RESULT`) instead of `{}`; helpers `_is_pending_remote_result(result)` / `_is_local_command_rejection(result)` collapse to single-key reads with no `PlayMode.is_network()` branch.  2 test assertions updated.  133 / 2 756 / 5 158 (unchanged).  MT-PHI.06e-3-sentinel pending (regression-only: hot-seat dial-token convert + Shift+D damage tooltip; network mode same paths). |
| I7 | Reconnection acceptance test + cleanup | ✅ commit `1c2b32c` — 5-test integration suite [tests/integration/test_reconnection_mid_attack.gd](tests/integration/test_reconnection_mid_attack.gd) validates the full `GameState.serialize → StateFilter.filter_for_player → GameState.deserialize → UIProjector.project` chain at `ATTACK_DEFENSE_TOKENS`: defender-reconnect renders interactive defense-tokens modal, attacker-reconnect renders the same modal read-only, payload carries dice state, OWNER-visible payloads are stripped from the opponent's view, no-flow yields empty intent.  I0 inventory file `docs/interaction_flow_inventory.md` deleted (obsolete — the parallel `NetworkInteractionState` channel is gone).  `scripts/lint_phase_i.sh` deleted (Phase-I freeze lint, no longer needed).  134 / 2 761 / 5 175 (+1 / +5 / +17). |
...
|-----|-------|--------------|----|
| **Defender cannot spend defense tokens on client screen** | _resolved I6b-3 R2 (2026-04-30)_ — defender peer drives spend & commit via `CommitDefenseCommand`; `AttackPanelMirror` renders the same `AttackSimPanel` and is interactive at `ATTACK_DEFENSE_TOKENS`; attacker-peer panel disabled in network mode. | I6b-3 R2 ✅ | MT-PHI.06b3-R2 ✅ |
| **Dice pool / dice results not rendered on `AttackPanelMirror`** | Networked attack: passive peer's mirror panel never shows dice count or rolled dice. R1b spec calls for dice-pool / dice-results population from `interaction_flow.payload`; payload already carries the data, but `AttackPanelMirror.apply_flow()` never calls `show_dice_count` / `show_dice_results`. | I6b-3 R1b follow-up (or fold into next slice) — extend `apply_flow()` to render `dice_pool` + `dice_results` from payload. | MT-PHI.06b3-R1b-dice |
| **Defender ship token visuals (shield pips / hull / token states) not refreshed on passive peer** | Networked attack: damage applied but the defender peer's ship token on the board does not update. `_handle_remote_resolve_damage` re-emits `ship_shields_changed` / `ship_hull_changed` / `ship_defense_token_changed`, but the on-board `ship_token` visual layer may not subscribe; only `ship_card_panel` updates. | Investigate `ship_token.gd` signal subscriptions; bind to the EventBus signals if missing. | MT-PHI.06b3-token-visuals |
...
| Network: passive-peer auto-resolve damage cards don't refresh visuals | Minor (visual only) | 2026-05-01 | Faceup damage cards with auto-resolve immediate effects (Structural Damage, Projector Misaligned, Life Support Failure) mutate the authoritative state on both peers via the broadcast `ResolveImmediateEffectCommand`, but `GameManager._handle_remote_immediate_effect` only emits `command_dials_changed` + `ship_defense_token_changed` — the `damage_card_flipped` / shield / hull / speed signals that drive `ShipCardPanel` and `ship_token` overlays are emitted only on the attacker peer's `_emit_immediate_signals`. Will be closed by an R5 follow-up that mirrors `_emit_immediate_signals` on the passive peer (or extracts it to a shared helper). |
