...
func _populate_shortcut_entries(vbox: VBoxContainer) -> void:
	...
	_add_line(vbox, "Left-click token", "Preview / commit reposition")
	_add_line(vbox, "Escape", "Cancel preview or damage picker")
	_add_line(vbox, "HUD readout", "Canonical X / Y / rotation")
	_add_separator(vbox)
	...
	_add_line(vbox, "Shift + R", "Save replay file")
