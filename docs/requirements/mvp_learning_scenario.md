...
| CP-009 | During the first round of the learning scenario, players may optionally use **pre-assigned (fixed) command dials** instead of selecting them manually. When enabled (`use_fixed_round1_commands: true` in the scenario JSON), the system auto-assigns the following commands (first listed = top of stack = revealed first; last listed = bottom = revealed last): **CR90 Corvette A:** Repair. **Nebulon-B Escort Frigate:** Navigate (top), Squadron (bottom). **Victory II-class Star Destroyer:** Repair (top), Navigate (middle), Concentrate Fire (bottom). The per-ship command arrays are stored in the scenario JSON under `fixed_round1_commands`, keyed by ship key. | LTP p.10 "suggested commands"; User req |
| CP-010 | When fixed round-1 commands are active (CP-009), the **entire command phase is skipped** in round 1: no command dial picker UI, no handoff screen, no player input. The game transitions directly from round start to the Ship Phase. All ships begin round 1 with their full complement of pre-assigned dials already in their command dial stacks. The `command_phase_complete` signal is still emitted so downstream listeners remain consistent. | User req; LTP p.10 |
...
| ID | Requirement | Notes |
|----|-------------|-------|
| DBG-020 | While moving, if the selected token's footprint at the mouse-cursor position would overlap another token, the selected token is displayed at the **closest legal position** whose centre has the **minimum Euclidean distance to the mouse cursor**, such that no overlap exists with any other token or play-area edge. The token must never penetrate another token's footprint. In debug mode, deployment zone boundaries are **not** treated as constraints (see DBG-032). | Token-token and play-area constraints always enforced; deployment zone is advisory in debug mode |
...
| DBG-032 | **Debug mode:** Deployment zone boundaries are **advisory only** — tokens may be dragged freely across the blue line. The deployment zone clamping logic (`_apply_deploy_zone_ship`, `_apply_deploy_zone_circle`) must be **preserved** in `TokenMover` for use in the full game scope, where boundaries will act as hard collision walls. In debug mode, the clamping is bypassed by passing `enforce_deploy_zones = false`. | Faction-aware boundary; enforcement deferred to full game |
| DBG-033 | When a token is released or moved **outside** its faction's deployment zone during debug-mode dragging, a toast notification (via `TooltipManager`) is shown, e.g. "CR90 Corvette A is outside deployment zone". The toast fires once per zone crossing (not every frame). Only the currently dragged token is checked. | Advisory feedback; uses existing TooltipManager |
| DBG-034 | The deployment zone validation logic in `TokenMover` and `DeploymentZoneOverlay` must be preserved intact for reuse when full-game deployment zone enforcement is implemented. No validation code may be deleted — only bypassed via a flag. | Future-proofing |
