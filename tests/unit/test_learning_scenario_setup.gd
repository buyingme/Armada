...
## five tokens in the Learning Scenario: three ships and two squadrons.
##
...
func test_get_all_placements_returns_five_tokens() -> void:
	# Arrange / Act
	var placements: Array[TokenPlacement] = \
			_setup.get_all_placements()
	# Assert
	assert_eq(placements.size(), 5,
			"Learning Scenario has exactly 5 tokens (3 ships + 2 squadrons)")


func test_get_token_count_returns_five() -> void:
	assert_eq(_setup.get_token_count(), 5,
			"get_token_count() should return 5")
...
func test_get_squadron_placements_returns_two_squadrons() -> void:
	var squadrons: Array[TokenPlacement] = \
			_setup.get_squadron_placements()
	assert_eq(squadrons.size(), 2,
			"Two squadrons: X-wing and TIE Fighter")
