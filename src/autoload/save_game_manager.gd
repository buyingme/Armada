...
## damage deck) to a JSON file under [code]user://saves/[/code].
##
## Design:
...
## Directory under [code]user://[/code] where save files are stored.
...
func save_game(game_state: GameState, file_name: String = "quicksave") -> bool:
	var dir_path: String = "user://%s" % SAVE_DIR
...
func load_game(file_name: String = "quicksave") -> GameState:
	var file_path: String = "user://%s/%s%s" % [SAVE_DIR, file_name, SAVE_EXT]
