...
func validate(game_state: GameState) -> String:
	...
	var draws := maxi(0, 2-int(ship.current_shields[zone]))
...
func execute(game_state: GameState) -> Dictionary:
	...
	elif not OVERLAP.open_next_purpose_resolution(game_state,owner,index): return {}
...
func execute_with_application_result(game_state: GameState,result: Dictionary) -> Dictionary:
	...
	var ship := game_state.get_ship(int(payload["owner_player"]),int(payload["ship_index"])); var damage: Dictionary=result["damage_application"]; var zone:=str(payload["hull_zone"]); var old:=int(ship.current_shields[zone]); var draws:=maxi(0,2-old)
	var expected_changes: Array = []
	if old > 0: expected_changes=[{"zone":zone,"new_shields":maxi(0,old-2)}]
	if damage["shield_changes"] != expected_changes or int(damage["facedown_delta"]) != draws or int(damage["new_hull"]) != ship.ship_data.hull-ship.get_total_damage()-draws or bool(damage["destroyed"]) != (int(damage["new_hull"])<=0): return {}
	if old > 0: ship.current_shields[zone]=maxi(0,old-2)
	if draws > 0:
	...
	elif not OVERLAP.open_next_purpose_resolution(game_state,int(payload["owner_player"]),int(payload["ship_index"])): return {}
