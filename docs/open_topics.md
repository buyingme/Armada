...
| I6e-3 | Name the submitter-contract pattern (`result.is_empty()` + `is_network()`) at L1261 / L2651 | ✅ commit pending — added `_is_pending_remote_result(result)` and `_is_local_command_rejection(result)` helpers; replaced both call sites. 7 → 5 real branches. 132 / 2 737 / 5 120. MT-PHI.06e-3 pending (regression-only: hot-seat ship activation via dial-token convert + Shift+D debug damage; network host/client ship activation; verify activation modal opens correctly on both peers and Shift+D damage replicates). |
