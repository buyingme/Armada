...
| A1-04 | Split 8 oversized functions (55→72 funcs) | `src/ui/ship_card_panel.gd` | `_build_left_column`, `_build_card_image`, `_build_right_column`, `_register_entry`; `_compute_entry_width`, `_max_child_width`, `_max_child_width_control`; `_safe_connect`; `_try_ship_phase_activation`, `_log_ineligible_dial_click`; `_create_hidden_dial_rect`, `_create_revealed_dial_rect`; `_find_token_index_in_column`; `_build_active_dial_stack`, `_build_spent_marker_section`; `_create_faceup_damage_rect`, `_create_facedown_badge` | ✅ |
| A1-05 | Split oversized functions | `src/ui/action_toolbar.gd` | No oversized functions found (all ≤ 30 lines) | ✅ |
| A1-06 | Split `_build_ui()` (54→18 lines) | `src/ui/repair_panel.gd` | `_apply_panel_style`, `_create_content_vbox`, `_build_title_section`, `_build_points_section`, `_build_actions_section`, `_build_button_row`, `_build_dismiss_hint` | ✅ |
| A1-07 | Split `_build_ui()` (50→14 lines) | `src/ui/displacement_modal.gd` | `_apply_panel_style`, `_build_header`, `_build_squadron_rows`, `_build_commit_button` | ✅ |
