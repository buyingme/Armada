...
| TD-18 | ~~Legacy `EffectRegistry` bridges remain for final static-guard retirement.~~ | ~~Medium~~ | **Resolved by N23** — legacy runtime effect classes/factories and obsolete tests were deleted, `GameState.effect_registry` and resolver hook fallbacks were removed, and the Phase K/N lint guard now blocks production reintroduction of retired symbols and old hook strings. |
...
> **Last audit:** 2026-05-23 — Phase N16 audited Heavy, Escort, Counter, Bomber, and Swarm before keyword migration. N17 added the shared keyword foundation without wiring live gameplay behaviour, and N18-N22 implemented the live keyword rules: Heavy movement/ship-target legality, Escort blockers, Counter affordances, Swarm rerolls, and Bomber critical permission. Follow-ups verified Counter locking, debug scenario network selection/spawning, command-backed off-turn Counter ownership, squadron no-move activation completion, and network Swarm reroll affordances. Latest automated gates passed at 185 GUT scripts / 3 272 tests / 6 676 asserts, Phase K lint 0 violations, baseline traces passing, and user MT pass confirmed for N17-N22. The network replay fixture omits one old illegal engaged-squadron move now rejected by Heavy-aware command validation. Network replay gates on host/client state-hash equality rather than committed network fixtures.
