...
| TD-15 | Phase K guardrails are enforced, but residual allow-listed presentation branches and oversized legacy managers remain. | Medium | `scripts/lint_phase_k.sh` reports 0 violations and 8 explicit allow-listed branches after L2. Current tracked sizes: `game_board.gd` 1 462, `attack_executor.gd` 2 479, `game_manager.gd` 2 271, `save_game_manager.gd` 1 061, `command_router_adapter.gd` 100, `modal_router.gd` 236, `ship_activation_controller.gd` 1 404. Phase L/M work must add new behavior through focused helpers and keep modal/network decisions projected through `UIProjector`. |
...
> **Last audit:** 2026-05-14 — Phase K complete; Phase L2 activation modal projection implemented. Network replay gates on host/client state-hash equality rather than committed network fixtures.
