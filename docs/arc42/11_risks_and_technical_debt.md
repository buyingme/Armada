...
| TD-18 | Legacy `EffectRegistry` bridges remain for four movement-facing persistent damage cards and for Escort/Swarm keyword cleanup. | Medium | Phase N9-N11 removed Targeter Disruption and Bomber from legacy runtime rebuilds and centralised movement compatibility in `ManeuverRuleResolver`. N12-N15 migrate the movement cards; N16-N19 audit/remove the remaining keyword/effect bridge and add the static guard. |
...
> **Last audit:** 2026-05-23 — Phase N9-N11 moved Targeter Disruption and Bomber to RuleRegistry hooks, leaving 4 legacy persistent damage-card ids and no rebuilt legacy `BomberEffect`. Remaining maneuver hook compatibility now lives in `ManeuverRuleResolver` instead of scene/tool predicates, and maneuver damage warnings preview Ruptured Engine, Damaged Controls, and Thruster Fissure before commit. N9-N11 automated gates passed; MT pass confirmed 2026-05-23. Network replay gates on host/client state-hash equality rather than committed network fixtures.
