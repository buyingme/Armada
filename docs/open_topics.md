...
| I6b-4c | Squadron-displacement call-site swap: `_finalize_maneuver_execute` submits `StartDisplacementCommand`; `DisplacementController._on_committed` submits `CommitDisplacementCommand`. | I6b-4c-1 ✅ commit pending — both commands submitted alongside legacy direct call so `interaction_flow` reflects placement state on both peers; legacy `start()` still drives the modal so behaviour is unchanged. I6b-4c-2 (remove legacy direct call, drive modal from projection) ⏳. 133 / 2 754 / 5 158. MT-PHI.06b-4c-1 pending (regression-only: hot-seat + network displacement still works; replay logs show start_displacement before move_squadron writes and commit_displacement after). |
