...
| I6d (partial) | Migrated `_is_local_activation_modal_controller()` to read `gs.interaction_flow.controller_player` (via the projection model `UIProjector` uses). Removed dead `_interaction_controller_player` / `_has_interaction_controller` fields and their writes in `_on_command_executed_project_ui`. **Hot-seat regression fixed pre-commit:** initial draft used `gs.active_player` which doesn't exist on `GameState` (the field lives on `GameManager`); the silent integer coercion to 0 locked the activation modal whenever player 1 (initiative) activated. Both call sites now use `GameManager.get_active_player()`. `is_network()` branches in `game_board.gd`: **10 → 9**. Remaining branches gate divergent host/client logic; final trim to ≤ 3 deferred to I6e. | `src/scenes/game_board/game_board.gd` | MT-PHI.06d passed 2026-04-28 |
