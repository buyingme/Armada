...
| I6b-4c | Squadron-displacement call-site swap: `_finalize_maneuver_execute` submits `StartDisplacementCommand`; `DisplacementController._on_committed` submits `CommitDisplacementCommand`. | I6b-4c-1 ✅ committed `647af38` — both commands submitted alongside legacy direct call. I6b-4c-2 ✅ commit pending — OV-002 fix: in network mode the legacy direct `_displacement_controller.start()` is gated to hot-seat only; the modal opens on the squadron-owner peer via projection (`_open_displacement_modal_from_command` reacts to `start_displacement` broadcast). Maneuvering peer's End-Activation resume on `commit_displacement` broadcast via `_resume_after_remote_displacement`. Hot-seat behaviour unchanged. 133 / 2 754 / 5 158. MT-PHI.06b-4c-2 pending (network: modal opens on opponent peer; hot-seat: unchanged). |
| I6b-4d | Squadron-displacement cleanup: remove old `start()` API + per-squadron `move_squadron` submits + camera flip from inside `start()`. | ⏳ |
