...
| TD-18 | Legacy `EffectRegistry` bridges remain for Escort/Swarm keyword cleanup and final static-guard retirement. | Medium | Phase N12-N15 emptied `DamageCardEffectFactory.PERSISTENT_EFFECT_IDS` and moved the remaining movement damage cards to RuleRegistry yaw/observer hooks. N16-N19 audit/remove the remaining keyword/effect bridge and add the static guard. |
...
> **Last audit:** 2026-05-23 — Phase N12-N15 moved Thrust Control Malfunction, Ruptured Engine, Damaged Controls, and Thruster Fissure to RuleRegistry hooks, emptying the persistent damage-card legacy id list. Maneuver warnings now derive from serialized faceup damage state, and post-maneuver damage is queued as deterministic command follow-ups. N12-N15 automated gates passed; MT pass confirmed 2026-05-23. Network replay gates on host/client state-hash equality rather than committed network fixtures.
