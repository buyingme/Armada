...
| C6 | Sidebar becomes authoritative projection | `ActivationSidebar.populate(...)`, `ActivationSidebar.refresh()`, `ActivationSidebar._on_ship_activated(...)`, `ActivationSidebar._on_squadron_activated(...)` | 🔄 In progress: `src/ui/combat/activation_sidebar.gd` now refreshes from `GameManager.current_game_state`, rebuilds when authoritative unit count changes, syncs active highlight from `GameManager` activating references, and listens to `interaction_state_changed` + command/phase/round/active-player signals. Tests: `tests/unit/test_activation_sidebar.gd` (3 tests). | Host/client sidebar parity snapshot tests pending manual/network validation |
| C7 | Activation modal permission gates | `ActivationModal.open(...)`, `ActivationModal._update_step_display()`, `_on_attack_pressed()`, `_on_repair_pressed()`, `_on_squadron_pressed()`, `_on_end_activation_pressed()` | ✅ `src/ui/combat/activation_modal.gd`: added `set_interactable(is_enabled: bool)` with button-state application and non-controller handler guards. ✅ `src/scenes/game_board/game_board.gd`: added controller-aware `_update_activation_modal_interactivity()` and centralized `_configure_and_open_activation_modal()` so every activation-modal open/reopen path applies `local_player == controller_player` gating. Tests: `tests/unit/test_activation_modal.gd` (+3 C7 tests). | Both peers see same step; passive peer cannot trigger state changes |
| C8 | Squadron modal permission gates | `SquadronActivationModal._update_ui()`, `_on_move_pressed()`, `_on_attack_pressed()`, `_on_commit_move_pressed()`, `_on_done_pressed()` | ✅ `src/ui/combat/squadron_activation_modal.gd`: added `set_interactable(is_enabled: bool)`, UI/button disable propagation, and handler-level early returns for non-controller peers (including move/attack/skip/commit/done/close and click consumption). ✅ `src/scenes/game_board/squadron_phase_controller.gd`: added `set_modal_interactable(is_enabled: bool)` and applied state on modal create/open paths. ✅ `src/scenes/game_board/game_board.gd`: authority helper now updates both activation and squadron modals from interaction controller ownership. Tests: `tests/unit/test_squadron_activation_modal.gd` (+3 C8 tests). | Passive peer sees mirrored modal but cannot move/attack/commit |
| C9 | Attack timeline mirrored and gated | `AttackExecutor._on_target_locked(...)`, `AttackExecutor._on_network_dice_result(...)`, `AttackSimPanel` action handlers (`_on_roll_pressed`, `_on_accuracy_confirm`, `_on_defense_done`, `_on_redirect_done_pressed`) | Use a single authoritative interaction-state stream (no per-step EventBus signal fan-out). Add command-backed step transitions for activation/attack windows, publish server-owned `interaction_state` updates on each committed step with monotonic versions + `payload.requires_seq`, gate panel controls by `controller_player`, and remove local-only step advancement in network mode. Subtasks: C9a step publisher in `GameManager`; C9b activation-step transition commands/events wired to publisher; C9c panel/executor controller guards by `step_id`; C9d parity tests for host/client timeline equality. | Attacker/defender handoff is visible and enforceable at each step; both peers advance checkmarks only on applied interaction-state versions |
...
**C5 validation note (2026-04-22):** Two-instance network manual test now
confirms the score-header text switches correctly between "make your choices"
and "waiting for opponent's choice" for active/passive peers.
